...
public class UnitManagement : MonoBehaviour
{
    ...
    public List<GameObject> unitSpawners = new();
    public List<GameObject> deployFlags = new();
    public GameObject selectedUnitSpawner, deployFlag;
    ...
    {
        if (selectedUnitSpawner != null && selectedUnitSpawner != unitSpawners[unitIndex])
        {
            DeselectUnitSpawner();
        }
    ...
    private void SelectUnitSpawner(int unitIndex)
    {
        ...
        selectedUnitSpawner = unitSpawners[unitIndex];
        deployFlag.SetActive(true);
        selectedUnitSpawner.transform.localScale = new(1.2f, 1.2f, 1);
    }

    private void DeselectUnitSpawner()
    {
        if (selectedUnitSpawner != null)
        {
            selectedUnitIndex = -1;
            deployFlag.SetActive(false);
            selectedUnitSpawner.transform.localScale = new(1, 1, 1);
            selectedUnitSpawner = null;
        }
    }
